...
import net.sknv.engine.graph.HeightMapMesh;
...
public class Terrain {
	...
	public Terrain(int terrainSize, float scale, float minY, float maxY, String heightMapFile, String textureFile, int textInc) throws Exception {
		...
		for (int row = 0; row < terrainSize; row++) {
			for (int col = 0; col < terrainSize; col++) {
				...
				Phantom terrainBlock = new Phantom(heightMapMesh.getMesh(), heightMapMesh.getMaterial());
	...
	protected Vector3f[] getTriangle(Vector3f position, Box2D boundingBox, Phantom terrainBlock) {
		...
		triangle[1] = new Vector3f(boundingBox.x + col * cellWidth, getWorldHeight(row + 1, col, terrainBlock), boundingBox.y + (row + 1) * cellHeight);
		triangle[2] = new Vector3f(boundingBox.x + (col + 1) * cellWidth, getWorldHeight(row, col + 1, terrainBlock), boundingBox.y + row * cellHeight);
		if(position.z < getDiagonalZCoord(triangle[1].x, triangle[1].z, triangle[2].x, triangle[2].z, position.x)) {
			triangle[0] = new Vector3f(boundingBox.x + col * cellWidth, getWorldHeight(row, col, terrainBlock), boundingBox.y + row * cellHeight);
		} else {
			triangle[0] = new Vector3f(boundingBox.x + (col + 1) * cellWidth, getWorldHeight(row + 2, col + 1, terrainBlock), boundingBox.y + (row + 1) * cellHeight);
		}
