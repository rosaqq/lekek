...
public class GameLogic {
	...
	private final ArrayList<RayCast> rayCasts = new ArrayList<>();
	...
	public void initPhysicsEngine() {
		...
		scene.getColliders().stream().forEach(collider -> {
		...
		if(!usingTerminal) {
			...
			if(selectedItem.isPresent()) {
				...
				if(window.isKeyPressed(GLFW_KEY_J))
					selectedItem.get().rotateWorldEuclidean(new Vector3f(0, (float) Math.PI / 200, 0));
			}
		}
	...
	public void processTerminal(String input) {
		...
		switch (in[0]) {
			case "savescene":
				String sceneName;
				if(in.length > 1) sceneName = in[1];
				else sceneName = "unnamed";
				getScene().save(sceneName);
				break;
			case "loadscene":
				if(in.length == 2) sceneName = in[1];
				else {
			...
			case "clearitems":
				...
				break;
			case "removeitem":
				selectedItem.ifPresentOrElse(x -> {
					scene.removeItem(x);
			...
			case "rotateitem":
				if(in.length == 4) {
					selectedItem.ifPresentOrElse(x -> x.rotateEuclidean(new Vector3f(Float.parseFloat(in[1]), Float.parseFloat(in[2]), Float.parseFloat(in[3]))), () -> System.out.println("No item selected"));
			...
			case "additem":
				try {
					...
					if(in.length == 5)
						newItem.translate(new Vector3f(Float.parseFloat(in[2]), Float.parseFloat(in[3]), Float.parseFloat(in[4])));
